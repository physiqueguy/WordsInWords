...
class Tree(object): # Prefix-tree object https://en.wikipedia.org/wiki/Trie
    ...
    def get_anagrams(self, word): # Get output list of anagram sentences
        ...
        MIN_SENTENCE_LENGTH = len(word) # minimum length of anagram sentence
        MIN_WORD_LENGTH = 1          # minimum length of each word in sentence
...
def write_sentences(my_list):
    with open('pout.txt', 'w') as f:
        for item in my_list:
            print >> f, item

    
def main():
    print "Loading dictionary into tree..."
    ...
    result = unique(list(dictionary.get_anagrams(word))) # get_anagrams of user's word from tree
    ...
    write_sentences(result) # write anagrams to text file 
